...
export async function login() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      showScreen('app');
      await loadRooms(); // после успешного входа загружаем список комнат
    } else {
      ...
      elements.roomsList.querySelectorAll('.room-item').forEach(item => {
        item.onclick = () => {
          ...
          if (typeof window.__onRoomSelected === 'function') {
            window.__onRoomSelected(roomId, invite);
          }
        };
      });
    }
...
export async function joinByCode() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.room_id && typeof window.__onRoomSelected === 'function') {
        window.__onRoomSelected(data.room_id, data.invite_link || code);
      }
    } else {
